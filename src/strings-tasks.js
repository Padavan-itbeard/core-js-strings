...
function sumOfCodes(str) {
  ...
  return str.split('').reduce((acc, ch) => acc + ch.charCodeAt(), 0);
}
...
function countVowels(str) {
  ...
  return str
    .split('')
    .reduce((acc, ch) => (vowels.includes(ch) ? acc + 1 : acc), 0);
}
...
function isPalindrome(str) {
  const normalizeStr = str.toLowerCase().replace(/[^a-zA-Z]/g, '');
  const reverse = normalizeStr.split('').reverse().join('');

  return reverse === normalizeStr;
}
